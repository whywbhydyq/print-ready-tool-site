...
export function PrintSizeTool() {
  ...
  const printUnit = mode === 'pixels-to-print-size' ? 'in' : input.unit;
  ...
  const svg = result ? svgGuide('Print Size Guide', [`Size: ${round(printWidthValue ?? 0, 3)} × ${round(printHeightValue ?? 0, 3)} ${printUnit}`, `Pixels: ${Math.round(result.pxw)} × ${Math.round(result.pxh)} px`, `Effective DPI: ${round(result.edpi, 1)}`, `Formula: ${result.formula}`]) : '';
