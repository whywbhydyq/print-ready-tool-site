...
replace('src/components/CalculatorClient.tsx', "const svg = result ? svgGuide('Print Size Guide', [`Size: ${round(result.width, 3)} × ${round(result.height, 3)} ${input.unit}`, `Pixels: ${Math.round(result.pxw)} × ${Math.round(result.pxh)} px`, `Effective DPI: ${round(result.edpi, 1)}`, `Formula: ${result.formula}`]) : '';", "const svg = result ? svgGuide('Print Size Guide', [`Size: ${round(mode === 'pixels-to-print-size' ? result.widthIn : result.width, 3)} × ${round(mode === 'pixels-to-print-size' ? result.heightIn : result.height, 3)} ${mode === 'pixels-to-print-size' ? 'in' : input.unit}`, `Pixels: ${Math.round(result.pxw)} × ${Math.round(result.pxh)} px`, `Effective DPI: ${round(result.edpi, 1)}`, `Formula: ${result.formula}`]) : '';");
